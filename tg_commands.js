...
return   【${tf.toUpperCase()}】 ${fusion.emoji} ${fusion.label}   💰 Price: ${nf(price,2)}  |  📊 Vol: ${volTxt}   RSI: ${rsi} | MACD: ${macd} | ATR: ${atr}   Structure: support:${support} | resistance:${resistance}   Fusion Score: ${fusionScore}  .trim();
...
return Elliott MultiTF Heatmap\n + parts.join(" | ");
...
// heuristic: base 40 + abs(overallFusion)40 + (ell.confidence0.2)
...
🚀 ${report.symbol} — AI Trader (single-file, Option-B UI)
...
Overall Bias: ${biasEmoji} ${biasLabel} | Strength(Fusion): ${overallFusion}

Buy Prob: ${buyProb}% | Sell Prob: ${sellProb}%

🎯 LONG Targets (confidence)

${topLongs}

🎯 SHORT Targets (confidence)
...
Data: Multi-source (Binance Vision + Binance + Bybit/KuCoin/CB) | vSingleFile | UI: Option-B per TF
