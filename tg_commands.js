...
function fuseMLTFsImproved(mlList = []) {
  ...
  for (const m of available) {
    ...
    // use ml module TP estimates if present
    tps.push({
      tf: m.tf,
      tp: safeNum(m.tpEstimate || m.tp || 0),
      tp2: safeNum(m.tp2Estimate || 0),
  ...
  // combine primary ML TPs weighted by TF priority
  let tpSum = 0, tpW = 0, tp2Sum = 0, tp2W = 0, hedgeSum = 0, hedgeW = 0, confSum = 0, confW = 0;
  for (const t of tps) {
    ...
    if (isNum(t.tp2) && t.tp2 > 0) { tp2Sum += t.tp2 * includeW; tp2W += includeW; }
  ...
  const secondaryTP = tp2W > 0 ? tp2Sum / tp2W : null;
  ...
  return {
    ...
    secondaryTP: isNum(secondaryTP) ? Number(secondaryTP) : null,
...
function buildStableTargetsImproved(clusterTargets = [], mlFusion = null, price = 0, feats = {}) {
  ...
  // choose stable primary: prefer matching direction
  let primary = null;
  if (scored.length) {
  ...
  if (!primary && scored.length) primary = scored[0];

  // last fallback: ATR-based scenic target
  if (!primary) {
    primary = { tp: price + atr * 2.5, source: "ATR", confidence: 30, tqf: scoreTarget(price + atr * 2.5, price, 30, 50, atr) };
  }

  // hedge: find opposite best candidate
...
export async function buildAIReport(symbol = CONFIG.SYMBOL || "BTCUSDT", opts = {}) {
  try {
    ...
    // improved ML fusion (now returns primaryTP, secondaryTP, hedgeTP from ML estimates)
    const mlFusion = fuseMLTFsImproved(mlResults);

    // build stable targets with improved logic
    const feat15 = blocks.find(b=>b.tf==="15m");
    const clusterStable = buildStableTargetsImproved(allTargets, mlFusion, price, { atr: feat15?.indicators?.ATR, candles: feat15?.candles });

    // === NEW: 3-TP selection (TP1 = ML primary if available, else cluster primary)
    // TP1 (Primary) <- ML fused primaryTP if present; else clusterStable.primaryTP
    // TP2 (Secondary) <- clusterStable.primaryTP (the cluster/elliott-derived primary)
    // TP3 (Hedge) <- clusterStable.hedgeTP
    function makeThreeTPs(mlFusionObj, clusterObj, priceVal) {
      const out = { tpPrimary: null, tpPrimarySrc: null, tpPrimaryConf: 0, tpSecondary: null, tpSecondarySrc: null, tpHedge: null, tpHedgeSrc: null, combinedConfidence: 0 };
      // ML primary preference
      if (mlFusionObj && isNum(mlFusionObj.primaryTP) && mlFusionObj.primaryTP > 0) {
        out.tpPrimary = Number(mlFusionObj.primaryTP);
        out.tpPrimarySrc = "ML-FUSED";
        out.tpPrimaryConf = mlFusionObj.confidence || 0;
      }
      // cluster primary
      if (clusterObj && isNum(clusterObj.primaryTP) && clusterObj.primaryTP > 0) {
        out.tpSecondary = Number(clusterObj.primaryTP);
        out.tpSecondarySrc = clusterObj.primarySource || "Cluster";
      }
      // if ML primary not present, fallback primary <- cluster primary
      if (!out.tpPrimary && out.tpSecondary) {
        out.tpPrimary = out.tpSecondary;
        out.tpPrimarySrc = out.tpSecondarySrc;
        out.tpPrimaryConf = clusterObj.primaryConf || 40;
        out.tpSecondary = null; // keep only ML-primary and cluster as secondary; if they are same then null later
      }
      // Hedge from cluster
      if (clusterObj && isNum(clusterObj.hedgeTP)) {
        out.tpHedge = Number(clusterObj.hedgeTP);
        out.tpHedgeSrc = clusterObj.hedgeSource || "Cluster";
      }
      // If ML provided a secondaryTP (from mlFusion.secondaryTP) use it as ML raw secondary if clusterSecondary missing
      if (mlFusionObj && isNum(mlFusionObj.secondaryTP) && mlFusionObj.secondaryTP > 0 && !out.tpSecondary) {
        // avoid duplicate equal to primary
        if (!out.tpPrimary || Math.round(out.tpPrimary) !== Math.round(mlFusionObj.secondaryTP)) {
          out.tpSecondary = Number(mlFusionObj.secondaryTP);
          out.tpSecondarySrc = "ML-SECONDARY";
        }
      }

      // dedupe: if any two TPs resolve to same rounded value, drop the duplicate as secondary preference order: Primary (ML) > Secondary (Cluster/ML-secondary) > Hedge
      const round = v => (isNum(v) ? Math.round(v) : null);
      const rPrimary = round(out.tpPrimary), rSecondary = round(out.tpSecondary), rHedge = round(out.tpHedge);
      if (rPrimary && rSecondary && rPrimary === rSecondary) out.tpSecondary = null;
      if (rPrimary && rHedge && rPrimary === rHedge) {
        // if hedge equals primary prefer hedge (keep hedge) but keep secondary if exists, else set hedge null? better keep hedge as hedge, but avoid duplicate by nudging hedge source label
        // We'll keep hedge but mark in src
        out.tpHedgeSrc = `${out.tpHedgeSrc} (dup-with-primary)`;
      }
      if (rSecondary && rHedge && rSecondary === rHedge) out.tpHedge = null;

      // combined confidence heuristics
      const confs = [];
      if (out.tpPrimaryConf) confs.push(out.tpPrimaryConf);
      if (clusterObj && clusterObj.primaryConf) confs.push(clusterObj.primaryConf);
      const avgConf = confs.length ? Math.round(confs.reduce((a,b)=>a+b,0)/confs.length) : (mlFusionObj?.confidence || clusterObj?.primaryConf || 40);
      out.combinedConfidence = avgConf;

      return out;
    }

    const threeTPs = makeThreeTPs(mlFusion, clusterStable, price);

    // add ML raw fused TP also into stableTargets for traceability
    clusterStable.mlFused = mlFusion || null;
    ...
    // Apply ML + news influence (dampened) to overallFusion — but keep dominant chart signals
    overallFusion = clamp(overallFusion + (mlFusion?.confidence ? (mlFusion.confidence/100) * 0.18 : 0) + newsBoost * 0.10, -1, 1);
    ...
    const report = {
      ...
      stableTargets: clusterStable,
      threeTPs, // contains tpPrimary (ML preferred), tpSecondary (cluster), tpHedge
      mlAcc: mlAccObj,
      news,
...
export async function formatAIReport(report = {}) {
  try {
    ...
    const three = report.threeTPs || {};

    // Format the three TPs: Primary (ML-preferred), Secondary (cluster), Hedge
    const tpPrimary = isNum(three.tpPrimary) ? nf(three.tpPrimary, 2) : "N/A";
    const tpPrimarySrc = three.tpPrimarySrc || "N/A";
    const tpPrimaryConf = isNum(three.tpPrimaryConf) ? three.tpPrimaryConf : (mlFusion?.confidence || 0);

    const tpSecondary = isNum(three.tpSecondary) ? nf(three.tpSecondary, 2) : "N/A";
    const tpSecondarySrc = three.tpSecondarySrc || stable.primarySource || "Cluster/ML";

    const tpHedge = isNum(three.tpHedge) ? nf(three.tpHedge, 2) : "N/A";
    const tpHedgeSrc = three.tpHedgeSrc || stable.hedgeSource || "Cluster/ML";

    const mlPerTF = report.ml?.perTF || [];
...
TP1 (Primary): <b>${tpPrimary}</b> (src:${tpPrimarySrc} | conf:${tpPrimaryConf}%)
TP2 (Secondary): <b>${tpSecondary}</b> (src:${tpSecondarySrc})
TP3 (Hedge): <b>${tpHedge}</b> (src:${tpHedgeSrc})
Combined Confidence: ${three.combinedConfidence}%
