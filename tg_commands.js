// tg_commands.js — upgraded to use the new ml_module_v8_6.js (V11 Ultra under same filename)
// Exports: buildAIReport, formatAIReport, sendSplitReport

import CONFIG from "./config.js";
import ML from "./ml_module_v8_6.js"; // this now contains V11 Ultra
...
export async function buildAIReport(symbol = CONFIG.SYMBOL || "BTCUSDT", opts = {}) {
  try {
    ...
    // fusion per block (same as earlier)
...
export async function formatAIReport(report = {}) {
  try {
    ...
    const getBlock = (tf) => {
      ...
      return { sig: sigText, rsi, macd, vol: volTxt, atr, ell: ellPat, ellConf, s: (isNum(S) ? nf(S, 2) : (S || "N/A")), r: (isNum(R) ? nf(R, 2) : (R || "N/A")), tpLine, sl: finalSL, price: nf(b.price, 2) };
    };
    ...
    // ML Targets: show Primary & Hedge depending on ml.direction (primary = side implied by ML)
    ...
    if (ml && ml.direction) {
      if (String(ml.direction).toLowerCase().includes("bear")) {
        ...
        // hedge: buy TP -> pick best long candidate
        mlHedge = (report.longs && report.longs[0]) ? nf(report.longs[0].tp, 2) : (isNum(ml.tpEstimate) ? nf(ml.tpEstimate, 2) : "N/A");
        ...
        mlHedge = (report.shorts && report.shorts[0]) ? nf(report.shorts[0].tp, 2) : (isNum(ml.tpEstimate) ? nf(ml.tpEstimate, 2) : "N/A");
      } else {
        // Neutral: choose nearest long as hedge and nearest short as primary both shown
...
ML Targets:
• Primary TP: <b>${mlPrimary}</b>  
• Hedge TP: <b>${mlHedge}</b>
━━━━━━━━━━━━━━━━━━
