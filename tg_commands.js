...
return   <b>【${tf.toUpperCase()}】 ${fusion.emoji} ${fusion.label}</b>   💰 Price: <b>${nf(price)}</b> | 📊 Vol: ${vol?.status || "N/A"}   RSI: <b>${nf(ind.RSI)}</b> | MACD: <b>${nf(ind.MACD?.hist, 4)}</b> | ATR: <b>${nf(ind.ATR)}</b>   Structure: support:${nf(ellSummary?.support || fib?.lo)} | resistance:${nf(ellSummary?.resistance || fib?.hi)}   Fusion Score: ${fusionScore}  .trim();
}
