// ml_module_v12_5.js
// ML v12.5 — Ultra-Balanced Precision (single-file drop-in)
// Exports:
...
// Default export includes all above (common shape used by your tg command)

import fs from "fs";
import path from "path";

/* ----------------- SAFE optional imports (repo-specific) -----------------
   fetchMultiTF: should return an object like { "15m": { data:[candles], price:123 }, ... }
   analyzeElliott: takes candles array -> returns { targets: [{tp, confidence, source}], pivots:[], confidence }
   News: module with fetchNewsBundle(symbol) returning { ok:true, sentiment:0.45, impact:"low", items:[], headline:"..." }
   If unavailable, module will use safe fallbacks.
--------------------------------------------------------------------------- */
let fetchMultiTF = null;
let analyzeElliott = null;
let News = null;
try { // try to require relative modules — if not found, use fallbacks
  // note: using dynamic import while keeping ESM style portability
  // If your environment doesn't support dynamic import, replace top-level imports manually.
  // We'll attempt to import; if fails, create safe fallback functions below.
  // (These dynamic imports won't crash at runtime if files missing because we catch)
} catch (e) {}

/* Fallback implementations if user doesn't have these modules (keeps module robust) */
const fallbackFetchMultiTF = async (symbol, tfs = ["1m","5m","15m","30m","1h"]) => {
  // Minimal placeholder: return empty data; callers must handle empty
  const out = {};
  for (const tf of tfs) out[tf] = { data: [], price: 0 };
  return out;
};
const fallbackAnalyzeElliott = async (candles) => {
  // Very light placeholder
  return { targets: [], pivots: [], confidence: 0, patterns: [] };
};
const fallbackFetchNews = async (symbol) => ({ ok:false, sentiment:0.5, impact:"low", items:[], headline:"No news" });

/* Try to import real modules if they exist (non-fatal) */
async function tryLoadOptionalDependencies() {
  try {
    // dynamic import style — works on node with ESM; if fails, ignore
    const utils = await import(pathToFile("./utils.js")).catch(()=>null);
    if (utils && typeof utils.fetchMultiTF === "function") fetchMultiTF = utils.fetchMultiTF;
  } catch(e){}
  try {
    const ellMod = await import(pathToFile("./elliott_module.js")).catch(()=>null);
    if (ellMod) analyzeElliott = ellMod.analyzeElliott || ellMod.default?.analyzeElliott || ellMod.default || null;
    if (typeof analyzeElliott !== "function") analyzeElliott = null;
  } catch(e){}
  try {
    const newsMod = await import(pathToFile("./news_social.js")).catch(()=>null);
    if (newsMod) News = newsMod.default || newsMod;
  } catch(e){}
}
// helper to normalize relative path for dynamic import (works in many setups)
function pathToFile(rel) {
  try { return new URL(rel, `file://${process.cwd()}/`).href; } catch(e) { return rel; }
}

// initialize optional deps asynchronously but allow synchronous code to fall back safely
tryLoadOptionalDependencies();

// safe accessors (use real functions if loaded, else fallback)
const _fetchMultiTF = async (symbol, tfs) => {
  if (typeof fetchMultiTF === "function") {
    try { return await fetchMultiTF(symbol, tfs); } catch(e) { return await fallbackFetchMultiTF(symbol, tfs); }
  }
  return await fallbackFetchMultiTF(symbol, tfs);
};
const _analyzeElliott = async (candles) => {
  if (typeof analyzeElliott === "function") {
    try { return await analyzeElliott(candles); } catch(e) { return await fallbackAnalyzeElliott(candles); }
  }
  return await fallbackAnalyzeElliott(candles);
};
const _fetchNews = async (symbol) => {
  if (News && (typeof News.fetchNewsBundle === "function")) {
    try { return await News.fetchNewsBundle(symbol); } catch (e) { return await fallbackFetchNews(symbol); }
  }
  return await fallbackFetchNews(symbol);
};

/* ---------------- persistence paths ---------------- */
const LOG_DIR = process.env.ML_LOG_DIR || path.join(process.cwd(), ".ml_v12_5_logs");
const PRED_FILE = path.join(LOG_DIR, "predictions_v12_5.json");
const OUT_FILE = path.join(LOG_DIR, "outcomes_v12_5.json");
const STATS_FILE = path.join(LOG_DIR, "stats_v12_5.json");
...
export function resetStats() { _stats = { total:0,wins:0,losses:0,accuracyCache:null,adaptiveWeights:{ w_ind:0.45,w_cnn:0.25,w_of:0.2,w_news:0.1,lr:0.02 },alerts:[],lastUpdated:null }; writeJsonSafe(STATS_FILE, _stats); return _stats; }

/* ---------------- core heuristic functions (indicators, features) ---------------- */
/* RSI (period n) - expects candles [{close},...] */
function computeRSI(closes, n = 14) {
  ...
  const avgGain = mean(gains.slice(-n)) || 0;
  const avgLoss = mean(losses.slice(-n)) || 0;
  if (avgGain + avgLoss === 0) return 50;
  const rs = avgGain / Math.max(EPS, avgLoss);
  return 100 - (100 / (1 + rs));
}
function computeStochRSI(closes, n=14) {
  // normalized RSI oscillator
  const rsiSeries = [];
  for (let i = 0; i < closes.length; i++) {
    const slice = closes.slice(Math.max(0, i - n + 1), i+1);
    rsiSeries.push(computeRSI(slice, n));
  }
  const last = rsiSeries.at(-1) ?? 50;
  const minR = Math.min(...rsiSeries.slice(-n));
  const maxR = Math.max(...rsiSeries.slice(-n));
  const denom = Math.max(EPS, maxR - minR);
  return ((last - minR) / denom) * 100;
}
function computeEMA(closes, period=20) {
...
function computeATR(candles, period=14) {
  ...
  return mean(trs.slice(-period)) || 0;
}
function computeMACD(candles) {
  // lightweight MACD: difference of EMA12 and EMA26 on closes, hist on last few
  const closes = candles.map(c=>Number(c.close||0));
  if (closes.length < 5) return { macd:0, signal:0, hist:0 };
  const ema12 = computeEMA(closes, 12) || 0;
  const ema26 = computeEMA(closes, 26) || 0;
  const macd = ema12 - ema26;
  // signal approx by smoothing
  const signal = macd * 0.25 + macd * 0.75; // trivial fallback (we don't maintain series)
  const hist = macd - signal;
  return { macd, signal, hist };
}

/* ---------------- candle vision heuristic (cnn fallback) ---------------- */
function candleVisionHeuristic(candles, lookback=8) {
  ...
  const closes = last.map(c => Number(c.close||0));
  const up = last.filter(c => c.close > c.open).length;
  const down = last.filter(c => c.close < c.open).length;
  const momentum = (closes.at(-1) - closes[0]) / Math.max(EPS, closes[0]);
  ...
  if (lower > body * 1.6 && upper < body * 0.6) score += 0.10;
  if (upper > body * 1.6 && lower < body * 0.6) score -= 0.10;
...
function chooseStablePrimaryHedge(candidates, dir="Neutral", price=0, feats={}, config={}) {
...
function pickSLForPrimary(dir="Bullish", price=0, feats={}) {
  const atr = Math.max(feats?.atr || 0, Math.abs(price) * 0.0005, 1);
...
function buildDecisionFeaturesForPrecision(featsPrimary = {}, newsObj = {}, systemProb = 0.5) {
...
export async function runMLPrediction(symbol = "BTCUSDT", tfc = "15m", opts = {}) {
  try {
    ...
    const mtfRaw = await _fetchMultiTF(symbol, tfsToFetch).catch(()=> ({}));
    ...
    if (!candles || candles.length < 6 || price <= 0) {
      return { modelVersion:"ml_module_v12_5", symbol, tf:tfc, direction:"Neutral", probs:{ bull:33.33, bear:33.33, neutral:33.33 }, maxProb:33.33, tpEstimate:null, tpSource:null, tpConfidence:0, slEstimate:null, perTf:[] };
    }

   // features + indicators
    const closesAll = candles.map(c => Number(c.close||0));
    const feats = (function(){ // compose feature object
      const slope = (() => {
        const n = Math.min(20, candles.length);
        let xmean=0,ymean=0; for (let i=0;i<n;i++){ xmean += i; ymean += closesAll[closesAll.length-n+i]; } xmean/=n; ymean/=n;
        let num=0, den=0; for (let i=0;i<n;i++){ const x=i; const y=closesAll[closesAll.length-n+i]; num += (x-xmean)*(y-ymean); den += (x-xmean)*(x-xmean); }
        return den === 0 ? 0 : num/den;
      })();
      const atr = computeATR(candles);
      const rsi = computeRSI(closesAll);
      const stochR = computeStochRSI(closesAll);
      const ema20 = computeEMA(closesAll, 20);
      const ema50 = computeEMA(closesAll, 50);
      const ema200 = computeEMA(closesAll, 200);
      const mom3 = closesAll.length >=4 ? (closesAll.at(-1) - closesAll[closesAll.length-4]) / Math.max(EPS, closesAll[closesAll.length-4]) : 0;
      const avgVol = mean(candles.map(c => Number(c.volume||0)).slice(-20));
      const lastVol = Number(candles.at(-1)?.volume || 0);
      const of = (function(){
        const last = candles.at(-1); const prev = candles.at(-2);
        if (!last || !prev) return {};
        const delta = (last.close - last.open) * (last.volume || 1);
        const vel = last.close - prev.close; return { delta, vel, lastVol, avgVol };
      })();
      return { close: closesAll.at(-1), slope, atr, rsi, stochR, ema20, ema50, ema200, mom3, avgVol, lastVol, of, candles };
    })();

    const indLayer = (function(){ // indicator scores
      const indScore = (function(){
        // indicator fusion small heuristic to produce 0..1 score
        let s = 0.5;
        s += clamp((feats.rsi - 50)/100 * 0.4, -0.2, 0.2);
        const emaStack = (feats.ema20 && feats.ema50 && feats.ema200) ? ((feats.ema20 > feats.ema50 && feats.ema50 > feats.ema200) ? 0.20 : (feats.ema20 < feats.ema50 && feats.ema50 < feats.ema200) ? -0.20 : 0) : 0;
        s += emaStack;
        const macd = computeMACD(candles);
        s += clamp(Math.tanh(macd.hist / Math.max(1, feats.atr || 1)) * 0.25, -0.25, 0.25);
        s = clamp(s, 0.01, 0.99);
        return { score: s, rsi: Math.round(feats.rsi), stoch: Math.round(feats.stochR), ema20: feats.ema20, ema50: feats.ema50, ema200: feats.ema200, atr: feats.atr, macd };
      })();
      return indScore;
    })();

    const cnn = await (async ()=>{ const cv = candleVisionHeuristic(candles, 8); return { score: cv.score, probs: cv.probs, features: cv.features }; })();
    const ofScore = orderFlowScore(feats.of || {});
    const newsObj = await _fetchNews(symbol).catch(()=>({ ok:false, sentiment:0.5, impact:"low" }));
    const newsScore = (newsObj && typeof newsObj.sentiment === "number") ? clamp(newsObj.sentiment, 0, 1) : 0.5;

    // per TF ML-like snapshots for fusion
    ...
    for (const tf of CONFIG.fusedTFs) {
      ...
      const cnnL = await (async ()=>{ const cv = candleVisionHeuristic(c,8); return { score: cv.score, probs: cv.probs }; })();
      ...
      // candidate TP using ell
      let ell = null;
      try { ell = await _analyzeElliott(c); } catch(e) { ell = null; }
    ...
    // main fusion
    const scoresMain = { ind: indLayer.score, cnn: cnn.score || 0.5, of: ofScore, news: newsScore };
    ...
    // build combined target candidates (elliott across fusedTFs + primary + ATR)
    let combinedCandidates = [];
    for (const tf of CONFIG.fusedTFs) {
      const raw = mtfRaw[tf] || { data:[], price:0 };
      const c = Array.isArray(raw.data) ? raw.data : [];
      if (!c || c.length < 6) continue;
      try { const ell = await _analyzeElliott(c); if (ell && Array.isArray(ell.targets) && ell.targets.length) combinedCandidates.push(...buildCandidateTPsFromElliott(ell)); } catch(e){}
    }
    try { const ellP = await _analyzeElliott(candles); if (ellP && Array.isArray(ellP.targets) && ellP.targets.length) combinedCandidates.push(...buildCandidateTPsFromElliott(ellP)); } catch(e){}
...
export async function runMicroPrediction(symbol = "BTCUSDT", tfc = "1m") {
  try {
    const mtf = await _fetchMultiTF(symbol, [tfc]).catch(()=>({}));
    const candles = Array.isArray(mtf[tfc]?.data) ? mtf[tfc].data : [];
    if (!candles || candles.length < 3) return { modelVersion:"ml_module_v12_5-micro", label:"Neutral", prob:33.33, reason:"insufficient" };
    const f = buildFeaturesFromCandles(candles);
    const ind = indicatorLayer(f);
    const cnn = candleVisionHeuristic(candles, 8);
    ...
    return { modelVersion:"ml_module_v12_5-micro", label, prob: Math.max(probBull, probBear), probBull, probBear, raw:{ ind: ind.score, cnn: cnn.score, of } };
...
/* ---------------- fuseMLTFs and buildStableTargets (exposed) ---------------- */
...
export function buildStableTargets(clusterTargets, mlFusion, price, feats) {
  try {
    ...
    const chosen = chooseStablePrimaryHedge(ded, (mlFusion && mlFusion.direction) || "Neutral", price, feats || {}, {});
    return { primaryTP: chosen.primary, hedgeTP: chosen.hedge, primarySource: chosen.primarySource, hedgeSource: chosen.hedgeSource, primaryConf: chosen.confidence, direction: (mlFusion && mlFusion.direction) || "Neutral" };
  } catch(e) {
    const fallback = chooseStablePrimaryHedge([], "Neutral", price, feats || {}, {});
    return { primaryTP: fallback.primary, hedgeTP: fallback.hedge, primarySource: fallback.primarySource, hedgeSource: fallback.hedgeSource, primaryConf: fallback.confidence, direction: "Neutral" };
  }
}

/* ---------------- trainAdaptive (exposed) ---------------- */
...
export async function buildAIReport(symbol="BTCUSDT", opts={}) {
  try {
    ...
    const mtfRaw = await _fetchMultiTF(symbol, tfs).catch(()=> ({}));
    const blocks = [];
    for (const tf of tfs) {
      ...
      const closes = candles.map(c=>Number(c.close||0));
      const ind = {
        RSI: candles.length ? Math.round(computeRSI(closes)) : 50,
      ...
      let ell = null;
      try { ell = await _analyzeElliott(candles); } catch(e) { ell = null; }
      const ellSummary = (()=>{ try { const pivots = ell?.pivots||[]; const lastLow = [...pivots].reverse().find(p=>p.type==="L") || null; const lastHigh = [...pivots].reverse().find(p=>p.type==="H") || null; return { support: lastLow?.price ?? null, resistance: lastHigh?.price ?? null, confidence: ell?.confidence ?? null }; } catch(e){ return { support:null,resistance:null,confidence:0 }; } })();
      let targets = [];
      if (ell && Array.isArray(ell.targets) && ell.targets.length) targets = ell.targets.map(t => ({ tp: Number(t.tp ?? t.target ?? t.price ?? 0), confidence: Math.round(Number(t.confidence ?? ell.confidence ?? 40)), source: t.source || t.type || tf })).filter(t => isNum(t.tp) && t.tp>0);
      else {
...
const price = blocks.find(x => x.tf === "15m")?.price 
    ...
    let mlFusion = fuseMLTFs(mlPerTF);
    if (!mlFusion || typeof mlFusion !== "object") {
      mlFusion = { direction: "Neutral", confidence: 0 };
    }

    // stable target builder
    const feat15 = blocks.find(b => b.tf === "15m")
      ? { atr: blocks.find(b => b.tf === "15m").indicators.ATR,
          candles: blocks.find(b => b.tf === "15m").candles }
      : {};
    ...
    // pro meters
    const proMeters = {
      rebound: computeReboundProbability(symbol, blocks),
      exhaustion: computeTrendExhaustion(symbol, blocks),
      volCrush: computeVolatilityCrush(symbol, blocks),
    ...
    const report = {
      ...
      news: await fetchNewsBundle(symbol).catch(() => ({ ok: false, sentiment: 0.5 })),

      buyProb: Number((((overallFusion + 1) / 2) * 100).toFixed(2)),
      sellProb: Number((100 - (((overallFusion + 1) / 2) * 100)).toFixed(2)),

      defaultSLLong: isNum(price)
        ? Number((price - (feat15.atr || price * 0.005) * 2).toFixed(8))
        : null,

      defaultSLShort: isNum(price)
        ? Number((price + (feat15.atr || price * 0.005) * 2).toFixed(8))
        : null
    };
